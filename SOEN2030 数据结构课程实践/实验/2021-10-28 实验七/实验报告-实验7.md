...
定义函数min_index，接受一个int数组作为输入，输出数组中最小元素的下标。

每次调用min_index找出输入数组costTime中最小值的下标加入数组order，并将找到的最小值更改为$\infty$（用0x7FFFFFFF表示）。当所有值都被改为$\infty$后，order即为所求结果。根据order计算总时间即可。
...
public class Exercise_2 {
    ...
    public static void main(String[] args) throws Exception {
        int[] costTime = { ... }; // costTime的具体值将在“输入”中给出
        // get order
...
定义函数has_repeated，输入数组arr、起始下标start和结束下标end，查找数组元素是否重复。
...
定义函数is_prime，接受一个整数，返回一个布尔量表示是否是素数。
